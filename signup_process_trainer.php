...
$sql = "INSERT INTO trainer (name, email, password_hash, schedule) VALUES (?, ?, ?, ?)";    // SQL statement for inserting new users into the database with sample values to start.
...
$stmt->bind_param("ssss", $_POST["name"], $_POST["email"], $password_hash, $_POST["schedule"]);  // Get the parameters of the form to replace the placeholders in the above statement. The 3 's' are the 3 strings (username, password, email).
